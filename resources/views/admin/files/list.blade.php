...
@section('content')
    ...
    @if (session('success'))
        <div class="mx-3 min-[390px]:mx-4 sm:mx-6 lg:mx-8 mt-4 mb-1 flex items-center gap-2 rounded-lg bg-green-50 border border-green-200 px-4 py-3 text-sm text-green-700">
    ...
    @if (session('error'))
        <div class="mx-3 min-[390px]:mx-4 sm:mx-6 lg:mx-8 mt-4 mb-1 flex items-center gap-2 rounded-lg bg-red-50 border border-red-200 px-4 py-3 text-sm text-red-600">
    ...
    <div class="w-full my-6 bg-white p-4 sm:p-5 md:p-6 lg:p-7 xl:p-8 2xl:p-9">

    <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between lg:gap-6">

        <!-- Title -->
        <div class="min-w-0 shrink-0">
            <h1 class="text-xl sm:text-2xl lg:text-3xl font-bold text-[#B70F1D] leading-tight">
                Files
            </h1>
            <p class="mt-1 text-xs sm:text-sm text-gray-400 leading-relaxed">
                Manage and organize your files
            </p>
        </div>

        <!-- Controls: search + filter + button -->
        <div class="flex flex-col sm:flex-row sm:flex-wrap lg:flex-nowrap items-stretch sm:items-center gap-3 sm:gap-4 w-full lg:w-auto">

            <!-- Search -->
            <div class="relative w-full sm:flex-1 sm:min-w-[200px] sm:max-w-xs lg:max-w-none lg:w-64 xl:w-72 2xl:w-80 shrink-0">
                <i class="fa-solid fa-magnifying-glass absolute left-3.5 top-1/2 -translate-y-1/2 text-[#B70F1D]/70 text-sm pointer-events-none"></i>

                <input type="text" name="search" id="searchInput" value="{{ $search }}"
                    placeholder="Search by Image, Document, Reels"
                    class="block w-full h-11 pl-10 pr-4 text-xs sm:text-sm rounded-lg border border-[#B70F1D]/60 text-gray-600 placeholder-gray-400 bg-white focus:outline-none focus:ring-1 focus:ring-[#B70F1D] focus:border-[#B70F1D] transition-all duration-200" />
            </div>

            <!-- Type filter -->
            <div class="relative w-full sm:w-auto sm:min-w-[150px] shrink-0">
    <select name="type" id="typeFilterInput"
        class="w-full h-11 appearance-none border border-[#B70F1D]/60 text-[#B70F1D] text-xs sm:text-sm font-medium rounded-lg pl-4 pr-9 bg-white focus:outline-none focus:ring-1 focus:ring-[#B70F1D] focus:border-[#B70F1D] transition-all duration-200 cursor-pointer">
        <option value="">All Types</option>

        @foreach ($types as $mediaType)
            <option value="{{ $mediaType }}" {{ $type === $mediaType ? 'selected' : '' }}>
                {{ \Illuminate\Support\Str::plural(ucfirst($mediaType)) }}
            </option>
        @endforeach
    </select>
    ...
    <i class="fa-solid fa-angle-down absolute right-3.5 top-1/2 -translate-y-1/2 text-[#B70F1D] text-xs pointer-events-none"></i>
            ...
            <!-- Add media button -->
            <a href="{{ route('admin.media.add') }}" class="w-full sm:w-auto shrink-0">
                <button type="button"
                    class="w-full sm:w-auto h-11 inline-flex items-center justify-center gap-2 bg-[#B70F1D] hover:bg-[#9c0c18] active:bg-[#8f0b16] text-white text-xs sm:text-sm font-medium rounded-lg px-4 sm:px-5 shadow-sm hover:shadow-md active:scale-[0.98] transition-all duration-200 cursor-pointer whitespace-nowrap">
    ...
    @push('scripts')
        ...
        <script>
            ...
            document.addEventListener('DOMContentLoaded', function () {
                ...
                $('#typeFilterInput').on('change', function () {
                    loadDirectories();
                });
