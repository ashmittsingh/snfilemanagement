...
@section('content')
    ...
    <div class="flex justify-between mb-6 sm:my-8 bg-white p-4">
        <div>
            <h1 class="text-xl sm:text-2xl font-bold text-[#B70F1D] leading-none">Edit Media</h1>
            <p class="text-xs sm:text-sm text-gray-400 mt-1">Update media directory and files</p>
        </div>
    </div>
    ...
    <form class="flex flex-col gap-6 sm:gap-7" id="mediaForm" method="POST"
        action="{{ route('admin.media.update', $directory->id) }}" enctype="multipart/form-data">
                    ...
                    <div id="existingFiles" class="space-y-2">
                        @foreach ($directory->files as $file)
                            <div id="existing-file-{{ $file->id }}"
                                class="flex items-center justify-between gap-3 rounded-lg border border-gray-200 bg-gray-50 px-3 py-2.5">

                                <div class="min-w-0 flex items-center gap-3">

                                    @if ($directory->type === 'image' && filled($file->storage_url))
                                        <img src="{{ $file->storage_url }}" alt="{{ $file->file_name }}"
                                            class="w-12 h-12 rounded-lg object-cover border border-gray-200 shrink-0">
                                    @elseif ($directory->type === 'reel' && filled($file->bunny_play_url))
    <video
        src="{{ $file->bunny_play_url }}"
        class="w-12 h-12 rounded-lg object-cover border border-gray-200 shrink-0"
        muted
        autoplay
        loop
...
@else
                                ...
                                <button type="button" data-file-id="{{ $file->id }}"
                                    class="remove-existing-file shrink-0 text-red-500 hover:text-red-700">
            ...
            <div id="reelsUploadSection">
                ...
                <label for="uploadReels"
                    ...
                    <p id="supportedFormats" class="text-xs sm:text-[13px] text-gray-400 mt-1.5">
                        Supported formats: -
                    </p>
                </label>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            */
            ...
            const supportedFormats =
                document.getElementById('supportedFormats');
            ...
            */
            ...
            const selectedFilePreviewUrls = new Map();
            ...
            */
            ...
            document.addEventListener(
                ...
                function (event) {
                    ...
                    const button =
                        event.target.closest('.remove-existing-file');
            ...
            */
            ...
            function validateFiles(files = null) {
                ...
                if (totalFiles > rule.maxFiles) {

                    showFieldError(
                        'files',
                        `You can have maximum ${rule.maxFiles} files in this directory.`
                    );
            ...
            function updateUploadState() {
                ...
                */
                ...
                supportedFormats.textContent =
                    `Supported formats: ${rule.extensions.join(', ').toUpperCase()}`;
    ...
    selectedFiles.forEach(function (file, index) {
        ...
        */
        ...
        if (mediaType.value === 'image') {
            ...
            let previewUrl =
                selectedFilePreviewUrls.get(file);
...
mediaFiles.addEventListener(
    ...
    function (event) {
        ...
        /*
        |--------------------------------------------------------------------------
        | Add newly selected files to existing selected files
        |--------------------------------------------------------------------------
        */
        ...
        /*
        |--------------------------------------------------------------------------
        | Validate complete file selection
        |--------------------------------------------------------------------------
        */
...
if (removedFile) {
    ...
    const previewUrl =
        selectedFilePreviewUrls.get(removedFile);
...
selectedFiles.splice(
    ...
    1
            ...
            */
            ...
            window.updateMedia = async function () {
                ...
                */
                if (
                    ...
                    */
                    ...
                    submitText.textContent = 'Update Media';
